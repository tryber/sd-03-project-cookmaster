...
const recipeDetails = async (req, res) => {
  ...
  return res.render('recipes/details', { 
    recipe, owned: (req.user) ? ((recipe.uId === req.user.id) ? true : false) : false
  });
}
...
const newRecipeForm = async (_req, res) => {
...
const delRecipeForm = async (req, res) => {
  return res.render('recipes/delete', { message: null, id: req.params.id });
}

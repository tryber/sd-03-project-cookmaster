...
const listRecipes = async (req, res) => {
  const recipes = await recipeModel.allRecipes();
...
const searchRecipes = async (req, res) => {
  ...
  const recipes = await recipeModel.queryRecipe(q);
...
const updateRecipe = async (req, res) => {
  ...
  await recipeModel.updateRecipe(req.params.id, recipeName, ingredients, instructions);
...
const deleteRecipeForm = async (req, res) => {
  const { id } = req.user;
  const { userId } = await recipeModel.findById(req.params.id);
